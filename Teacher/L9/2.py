...
def get_data_ads(find_ads_first_page):
    ...
    for index, i in enumerate(find_ads_first_page[:10]):
        print(f"{index}/{len(find_ads_first_page)}")
        card_data = {}
...
def ads_on_all_pages(url):
    ...
    for page in range(1):
        ...
        with open('OLX.json', 'w', encoding='utf-8') as file:
            json.dump(list_ads_urls, file, ensure_ascii=False)
